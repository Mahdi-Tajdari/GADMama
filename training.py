...
def RunExp(args, dataset, data, Net, rb):
    ...
    def train(args, model, optimizer, data, dprate, rb):
        ...
        _, out_main = model(data.x, data.adj_t)
    ...
    for epoch in range(args.epochs):
        ...
        if args.early_stopping > 0 and epoch > args.early_stopping:
            tmp = torch.tensor(val_acc_history[-(args.early_stopping + 1):-1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, choices=['ogbn-proteins','ogbn-arxiv','AMiner', 'Reddit', 'CS', 'Physics', 'Corafull','Wisconsin', 'Cora','Citeseer','Pubmed','Computers','Photo','Chameleon','Squirrel','Actor','Texas','Cornell', 'Roman-empire', 'Amazon-ratings', 'Minesweeper', 'Tolokers', 'Questions'], default='Cora')
    parser.add_argument('--device', type=int, default=0, help='GPU device.')
    parser.add_argument('--runs', type=int, default=10**6, help='number of runs.')
    
 
    parser.add_argument('--net', type=str, default='GCN_mamba_Net')
